...
var chainEndpoint = "https://wax.eosusa.news";
...
$(window).load(function($loadEvent) {
  ...
  function updateEosData() {
      ...
      // WAX
      let waxPromise = new Promise((resolve, reject) => {
          let reqWax = getXmlHttpRequestObject();
          reqWax.open("GET", "https://api.coingecko.com/api/v3/simple/price?ids=wax&vs_currencies=eos"); 
          reqWax.onload = () => {
              resolve(reqWax);
          };
          reqWax.onerror = () => {
          ...
          .then(xhr => {
              let responseJson = JSON.parse(xhr.responseText);
              waxEosScalar = responseJson.wax.eos;
      ...
      Promise.all([globalPromise, eosPromise, waxPromise, ramPromise, banPromise])
          .then(() => {
              runCalculations();
              updatePage();
          });
  ...
  function runCalculations() {
      waxPriceUsd = waxEosScalar * eosPriceUsd;

      ramPriceUsd = ramPriceEos * waxPriceUsd;
      netPriceUsd = netPriceEos * waxPriceUsd;
      cpuPriceUsd = cpuPriceEos * waxPriceUsd;
  }

  function updatePage() {
      let waxTarget = $("#eos-price-usd");

      waxTarget.html(`1 WAX = $${waxPriceUsd.toFixed(2)} USD`);
      ...
      ramPriceEosElem.html(`${ramPriceEos} WAX per KiB`);

      let ramPriceElem = $("#ram-price-usd");
      ramPriceElem.html(`~ $${(ramPriceEos * waxPriceUsd).toFixed(3)} USD per KiB`);

      let netPriceEosElem = $("#net-price-eos");
      netPriceEosElem.html(`${netPriceEos} WAX/KiB/Day`);

      let netPriceUsdElem = $("#net-price-usd");
      netPriceUsdElem.html(`~ $${(netPriceEos * waxPriceUsd).toFixed(3)} USD/KiB/Day`);

      let cpuPriceEosElem = $("#cpu-price-eos");
      cpuPriceEosElem.html(`${cpuPriceEos} WAX/ms/Day`);

      let cpuPriceUsdElem = $("#cpu-price-usd");
      cpuPriceUsdElem.html(`~ $${(cpuPriceEos * waxPriceUsd).toFixed(3)} USD/ms/Day`);
  }
